...
public class DatPhongPanel extends JPanel {
    ...
    static class Room {
        ...
        boolean available;
        Room(String id, String type, String floor, boolean available) {
            this.id = id; this.type = type; this.floor = floor; this.available = available;
        }
    }
    ...
    private String activeTab = "All room(5)";
    ...
    private void initData() {
        allRooms.add(new Room("101", "Phòng thường", "Tầng 1", true));
        allRooms.add(new Room("201", "VIP",           "Tầng 2", true));
        allRooms.add(new Room("301", "VIP",           "Tầng 2", true));
        allRooms.add(new Room("401", "VIP",           "Tầng 4", true));
        allRooms.add(new Room("501", "Phòng thường",  "Tầng 5", true));
        allRooms.add(new Room("601", "VIP",           "Tầng 6", true));
        allRooms.add(new Room("701", "Phòng thường",  "Tầng 7", false));
    }
    ...
    private JPanel buildTabsAndDates() {
        ...
        // Date row
        JPanel dateRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 20, 0));
        dateRow.setOpaque(false);
        dateRow.setAlignmentX(LEFT_ALIGNMENT);
        dateRow.setMaximumSize(new Dimension(Integer.MAX_VALUE, 60));

        dateRow.add(buildDateField("Check in", "Tue, Mar 2"));
        dateRow.add(buildDateField("Check out", "Wed, Mar 10"));
    ...
    private JPanel buildDateField(String label, String value) {
        JPanel p = new JPanel();
        ...
        JPanel field = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 6)) {
            @Override protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                g2.setColor(WHITE);
        ...
        field.setPreferredSize(new Dimension(160, 36));
        field.add(lbl(value, F_PLAIN13, DARK));
        // Calendar icon
        JLabel calIcon = new JLabel() {
            @Override protected void paintComponent(Graphics g) {
                ...
                g2.drawRoundRect(cx-7, cy-6, 14, 13, 2, 2);
                g2.drawLine(cx-7, cy-3, cx+7, cy-3);
                g2.drawLine(cx-3, cy-9, cx-3, cy-5);
                g2.drawLine(cx+3, cy-9, cx+3, cy-5);
                g2.dispose();
            }
        };
        calIcon.setPreferredSize(new Dimension(18, 18));
        field.add(calIcon);
    ...
    private JPanel buildFilterBar() {
        ...
        // Dropdowns
        bar.add(dropDown("Loại phòng ▾", new String[]{"Loại phòng","Phòng thường","VIP"}));
        bar.add(dropDown("Tầng ▾", new String[]{"Tầng","Tầng 1","Tầng 2","Tầng 3","Tầng 4","Tầng 5"}));

        return bar;
    }

    private JPanel dropDown(String defaultText, String[] options) {
        ...
        btn.addMouseListener(new MouseAdapter() {
            @Override public void mouseClicked(MouseEvent e) {
                ...
                for (String opt : options) {
                    ...
                    item.addActionListener(ae -> lbl.setText(opt + " ▾"));
                    menu.add(item);
                }
    ...
    private JPanel buildRoomTable() {
        ...
        int[] widths = {100, 40, 80, 120, 80};
        for (int i = 0; i < cols.length; i++) {
            JLabel h = lbl(cols[i], F_BOLD12, GRAY);
    ...
    private JPanel buildRoomRow(Room room) {
        ...
        typeCell.add(lbl(room.type, F_PLAIN13, MID));
