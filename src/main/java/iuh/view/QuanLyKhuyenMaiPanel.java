...
public class QuanLyKhuyenMaiPanel extends JPanel {
                ...
                if (e.getClickCount() == 2) {
                    ...
                    if (row >= 0) {
                        ...
                        Request request = Request.builder()
                                .commandType(CommandType.GET_KHUYEN_MAI_BY_ID)
                                .object(maKhuyenMai)
                                .build();
                        Response response = clientConnection.sendRequest(request);
                        KhuyenMaiDTO km = (KhuyenMaiDTO) response.getObject();
                        if (km != null) {
                            openModal(km, false);
                        } else {
    ...
    private void refreshTable() {
        java.util.List<KhuyenMaiDTO> list = getCurrentData();
    ...
    private java.util.List<KhuyenMaiDTO> getCurrentData() {
        String keyword = getSearchKeyword();
        Request request;
        if (keyword.isBlank()) {
            request = Request.builder().commandType(CommandType.GET_ALL_KHUYEN_MAI).build();
        } else {
            request = Request.builder()
                    .commandType(CommandType.GET_KHUYEN_MAI_BY_KEYWORD)
                    .object(keyword)
                    .build();
        }
        Response response = clientConnection.sendRequest(request);
        return (java.util.List<KhuyenMaiDTO>) response.getObject();
    }
    ...
    private void openModal(KhuyenMaiDTO khuyenMai, boolean isNew) {
...
class KhuyenMaiModal extends JDialog {
    ...
    private final KhuyenMaiDTO current;
    ...
    KhuyenMaiModal(JFrame owner, KhuyenMaiDTO khuyenMai, boolean isNew, Runnable onChanged) {
        super(owner, isNew ? "Thêm khuyến mãi" : "Chi tiết khuyến mãi", true);
        this.isNew = isNew;
        this.current = khuyenMai == null ? new KhuyenMaiDTO() : khuyenMai;
        this.onChanged = onChanged;
        if (isNew) {
            Request request = Request.builder().commandType(CommandType.PHAT_SINH_MA_KHUYEN_MAI).build();
            this.autoGeneratedCode = (String) clientConnection.sendRequest(request).getObject();
        } else {
            this.autoGeneratedCode = this.current.getMaKhuyenMai();
        }
    ...
    private JPanel buildForm() {
        ...
        tfMa = field(autoGeneratedCode, false);
    ...
    private void onSave() {
        try {
            KhuyenMaiDTO km = collectFormData();
            Request request = Request.builder()
                    .commandType(CommandType.CREATE_KHUYEN_MAI)
                    .object(km)
                    .build();
    ...
    private void onUpdate() {
        try {
            KhuyenMaiDTO km = collectFormData();
            Request request = Request.builder()
                    .commandType(CommandType.UPDATE_KHUYEN_MAI)
                    .object(km)
                    .build();
            clientConnection.sendRequest(request);
    ...
    private void onDelete() {
        ...
        try {
            Request request = Request.builder()
                    .commandType(CommandType.DELETE_KHUYEN_MAI)
                    .object(tfMa.getText().trim())
                    .build();
            clientConnection.sendRequest(request);
    ...
    private KhuyenMaiDTO collectFormData() {
        String ma = isNew ? autoGeneratedCode : tfMa.getText().trim();
        ...
        String tongTienToiThieuText = tfTongTienToiThieu.getText().trim();
        String tongKhuyenMaiToiDaText = tfTongKhuyenMaiToiDa.getText().trim();

        if (tongTienToiThieuText.isBlank()) {
            throw new IllegalArgumentException("Tổng tiền tối thiểu không được để trống.");
        }
        if (tongKhuyenMaiToiDaText.isBlank()) {
            throw new IllegalArgumentException("Tổng khuyến mãi tối đa không được để trống.");
        }
