...
public class QuanLyKhuyenMaiPanel extends JPanel {
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 2) {
                    ...
                    if (row >= 0) {
                        ...
                        KhuyenMaiDTO km = (KhuyenMaiDTO) res.getObject();
                        if (km != null) {
                            openModal(km, false);
                        } else {
    ...
    private java.util.List<KhuyenMaiDTO> getCurrentData() {
        ...
        try {
            Request req = Request.builder()
                    .commandType(keyword.isBlank() ? CommandType.GET_ALL_KHUYEN_MAI : CommandType.GET_KHUYEN_MAI_BY_KEYWORD)
                    .object(keyword.isBlank() ? null : keyword)
                    .build();
            Response res = clientConnection.sendRequest(req);
            return (java.util.List<KhuyenMaiDTO>) res.getObject();
        } catch (Exception e) {
            e.printStackTrace();
...
class KhuyenMaiModal extends JDialog {
    ...
    KhuyenMaiModal(JFrame owner, KhuyenMaiDTO khuyenMai, boolean isNew, Runnable onChanged) {
        ...
        if (isNew) {
            try {
                Request req = Request.builder().commandType(CommandType.PHAT_SINH_MA_KHUYEN_MAI).build();
                Response res = clientConnection.sendRequest(req);
                this.autoGeneratedCode = (String) res.getObject();
            } catch (Exception e) {
